			...
			<div class="col-md-2 ">
				<ul class="list-group">
					<li class="list-group-item  text-center" id="channel"><a
						class="channel" href="${pageContext.request.contextPath}/">热门</a></li>

					<c:forEach items="${channels}" var="c">
						<li class="list-group-item text-center" id="channel${c.id }"><a
							class="channel" href="${pageContext.request.contextPath}/?channelId=${c.id}">${c.name}</a></li>
					</c:forEach>
				</ul>
			</div>
			<!-- 中间内容主体区 -->
						...
						<c:forEach items="${hotArticles}" var="c">
							<div class="media">
								...
								<div class="media-body">
									<h5 class="mt-0">
										<a href="${pageContext.request.contextPath}/article/select?id=${c.id }" target="_blank">${c.title }</a>
									</h5>
					...
					<div id="category">
						<ul class="nav">
							...
							<li class="nav-item" id="cat"><a class="nav-link"
								href="${pageContext.request.contextPath}/?channelId=${article.channelId}">全部</a></li>
							<c:forEach items="${categorys}" var="c">
								<li class="nav-item" id="cat${c.id }"><a class="nav-link"
									href="${pageContext.request.contextPath}/?channelId=${c.channelId}&categoryId=${c.id}">${c.name }</a></li>
							</c:forEach>
						</ul>
			...
			<div class="col-md-3">
						...
						<ol>
							<c:forEach items="${lasts}" var="article">
								<li class="text-truncate"><a
									href="${pageContext.request.contextPath}/article/select?id=${article.id }" target="_blank">${article.title}</a></li>
							</c:forEach>
						</ol>
					</div>
				</div>
				...
				<c:forEach items="${images}" var="m">
					<div class="card">
						...
						<div class="card-body">
							<a href="${pageContext.request.contextPath}/article/selectPic?id=${m.id }" target="_blank"><img
				...
				<div class="card">
					...
					<c:forEach items="${votes}" var="m">
						<div class="card-body">
							<a href="${pageContext.request.contextPath}/article/selectVote?id=${m.id }" target="_blank">
								${m.title } </a>
